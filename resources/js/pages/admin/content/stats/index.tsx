...
export default function AdminContentStatsIndex({ window, summary, topPosts }: Props) {
    return (
        <>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
                {[
                    ...
                    <Button
                        ...
                        size="sm"
                        onClick={() =>
                            router.get('/admin/content/stats', { window: item.value }, { preserveState: true })
                        }
                    >
                        {item.label}
                    </Button>
                        ...
                        <li key={row.post.id} className="rounded-lg border p-4">
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <Button variant="outline" size="sm" asChild>
                                    <Link href={`/admin/content/posts/${row.post.slug}/edit`}>Edit</Link>
                                </Button>
                            </div>
                        </li>
                    ))}
