            ...
            <div className="mt-6 space-y-4">
                {feedbacks.length === 0 ? (
                    ...
                    feedbacks.map((feedback) => (
                        <article key={feedback.id} className="rounded-lg border p-4 text-sm">
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-muted-foreground">
                                <span className="font-medium text-foreground">
                                    {feedback.userName ?? 'Unknown user'}
                                </span>
                                {feedback.userEmail && <span>{feedback.userEmail}</span>}
                                {feedback.teamName && <span>{feedback.teamName}</span>}
                            ...
                            <p className="mt-3 whitespace-pre-wrap text-foreground">{feedback.message}</p>
                        </article>
                    ))
                )}
            </div>
