...
class UpdatePodcastEpisodeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $podcastEpisode = $this->route('podcast_episode');
        ...
        return [
            ...
            'episode_number' => [
                ...
                'integer',
                'min:1',
                Rule::unique('podcast_episodes', 'episode_number')
                    ->where('podcast_show_id', $this->input('podcast_show_id'))
                    ->ignore($podcastEpisode->id),
            ],
            'title' => ['required', 'string', 'max:255'],
            'slug' => ['required', 'string', 'max:255', 'alpha_dash', Rule::unique('podcast_episodes', 'slug')->ignore($podcastEpisode->id)],
